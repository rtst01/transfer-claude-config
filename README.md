...
keybindings, statusline, MCP-серверы и список плагинов.
...
| `agents/`, `skills/`, `commands/` | `~/.claude/` (симлинки и junction'ы разыменовываются — уезжает содержимое) |
| `mcpServers` | только этот блок из `~/.claude.json` |
| список плагинов | `~/.claude/plugins/` — только идентификаторы, ставит `claude plugin` |

При `pull`/`import` недостающие плагины доустанавливаются официальным CLI
(`claude plugin marketplace add …` + `claude plugin install …`) — переносится
список, а не файлы. На нестандартном `CCSYNC_HOME` команды не выполняются
автоматически, а выводятся для ручного запуска.

**Никогда не трогается:** `settings.local.json` (машинно-локальное),
история, сессии, кэш, telemetry, кэш и файлы плагинов (переносится только их
список, восстановление — через `claude plugin install`).
...
- `mcpServers` — объединение по именам; на той же ОС побеждает входящее,
  при переносе между разными ОС — локальное (см. «Кроссплатформенность»);
...
- MCP-серверы: одноимённое локальное определение не затирается входящим с другой
  ОС — оно заведомо рабочее здесь. Новые сервера адаптируются: обёртка
  `cmd /c npx …` снимается на unix и надевается на Windows, виндовые переменные
  (`ProgramData`, `LOCALAPPDATA` и т.п.) вычищаются из env. Серверы-`.exe`
  на unix не применяются — вместо этого предупреждение;
...
ccsync doctor     # диагностика: git, bash, PATH, репо, хуки, плагины, карантин — с рецептами
