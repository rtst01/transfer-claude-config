...
/**
 * Возвращает "x.y.z" последнего релиза или null.
 * Основной способ — redirect github.com/…/releases/latest → …/tag/vX.Y.Z:
 * не тратит квоту GitHub API (анонимный API — всего 60 запросов/час).
 */
function fetchLatestVersion() {
  return new Promise((resolve) => {
    const req = https.get(
      `https://github.com/${REPO}/releases/latest`,
      { headers: { 'User-Agent': `ccsync/${VERSION}` } },
      (res) => {
        res.resume();
        const m = String(res.headers.location || '').match(/\/tag\/v?([\d.]+)/);
        if (m) return resolve(m[1]);
        // fallback: API (может упереться в rate limit)
        httpGet(`https://api.github.com/repos/${REPO}/releases/latest`)
          .then((body) => resolve((JSON.parse(body).tag_name || '').replace(/^v/, '') || null))
          .catch(() => resolve(null));
      }
    );
