...
/** Возвращает worktree к состоянию HEAD (после materialize в status/diff). */
...
/**
 * Обязательная конфигурация служебного репо:
 * - identity, иначе commit падает (CI, чистые машины);
 * - core.autocrlf=false: репо — байтовое зеркало; на Windows autocrlf=true
 *   даёт фантомные "M" в status (checkout CRLF vs записанные LF).
 */
function ensureRepoConfig(repoDir) {
  git(repoDir, ['config', 'core.autocrlf', 'false']);
  if (!git(repoDir, ['config', 'user.email']).stdout.trim()) {
    git(repoDir, ['config', 'user.name', 'ccsync']);
    git(repoDir, ['config', 'user.email', 'ccsync@local']);
  }
}
...
function init(url, opts = {}) {
  ...
  // .gitattributes: репо — байтовое зеркало, никаких eol-конвертаций.
  // Иначе на Windows checkout даёт CRLF, и status вечно показывает "M".
  fs.writeFileSync(path.join(repoDir, '.gitattributes'), '* -text\n');
  ensureRepoConfig(repoDir);
...
function materialize(repoDir) {
  ...
  fs.writeFileSync(
    path.join(repoDir, 'plugins.json'),
...
function bundleFromRepo(repoDir) {
  ...
  const plugins = readJsonSafe(path.join(repoDir, 'plugins.json'));
  return {
    meta,
    files,
    mcpServers: mcpServers && Object.keys(mcpServers).length ? mcpServers : null,
    plugins: plugins && Array.isArray(plugins.plugins) && plugins.plugins.length ? plugins : null,
  };
}
...
function pull(opts = {}) {
  const repoDir = requireRepo();
  // worktree мог остаться «грязным» после status/diff — чистим до pull
...
function status() {
  ...
  if (!lines.length) {
    ...
    log.dim('\nccsync diff — подробный дифф, ccsync push — отправить, ccsync pull — применить из репо');
  }
  restoreWorktree(repoDir);
}

/** ccsync diff — подробный дифф локального конфига против репозитория. */
function diffRepo() {
  const repoDir = requireRepo();
  materialize(repoDir);
  // meta.json — служебный timestamp, в дифф не входит
  const r = git(repoDir, ['-c', 'color.ui=always', 'diff', '--', 'claude', 'mcp-servers.json', 'plugins.json']);
  const out = r.stdout.trim();
  if (!out) log.ok('Отличий от репозитория нет');
  else console.log(out);
  restoreWorktree(repoDir);
}

/** ccsync log — история синхронизаций: кто, когда, что менял. */
function logCmd(count = 15) {
  const repoDir = requireRepo();
  const r = git(repoDir, [
    'log',
    `-${count}`,
  ...
  if (!r.stdout.trim()) {
    log.info('История пуста — сделай первый ccsync push');
    return;
  }
  console.log(r.stdout.trimEnd());
}

module.exports = { init, push, pull, status, diffRepo, logCmd, requireRepo };
