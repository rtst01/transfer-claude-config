...
// windows-переменные, бессмысленные в env MCP-серверов на unix
const WIN_ENV_KEYS = ['ProgramData', 'ProgramFiles', 'ProgramFiles(x86)', 'LOCALAPPDATA', 'APPDATA', 'SystemRoot', 'ComSpec'];

/**
 * Адаптирует определение MCP-сервера под текущую ОС при кросс-платформенном
 * переносе: снимает/надевает обёртку "cmd /c", чистит чужой env.
 * Возвращает null, если сервер непереносим (windows-бинарь на unix).
 */
function adaptMcpEntry(name, entry, report) {
  const e = JSON.parse(JSON.stringify(entry));
  if (!e.command) return e; // http/sse-сервер — переносим как есть

  if (!cfg.IS_WIN) {
    if (/^cmd(\.exe)?$/i.test(e.command) && Array.isArray(e.args) && /^\/c$/i.test(e.args[0] || '')) {
      e.command = e.args[1];
      e.args = e.args.slice(2);
    }
    if (/\.exe$/i.test(e.command || '')) {
      report.skipped.push(`mcpServers.${name} (windows-бинарь, на этой ОС не запустится: ${e.command})`);
      return null;
    }
    if (e.env) for (const k of WIN_ENV_KEYS) delete e.env[k];
  } else if (e.command === 'npx') {
    // на Windows npx без cmd-обёртки не спавнится
    e.args = ['/c', 'npx', ...(e.args || [])];
    e.command = 'cmd';
  }
  return e;
...
function apply(bundle, opts = {}) {
  ...
  // При переносе между разными ОС локальное определение с тем же именем побеждает:
  // оно заведомо рабочее для этой ОС, а входящее — под другую платформу.
  if (bundle.mcpServers && Object.keys(bundle.mcpServers).length) {
    const crossPlatform = bundle.meta.platform !== process.platform;
    const adaptedAll = walkStrings(bundle.mcpServers, makePathAdapter(srcHome, report));
    const claudeJson = readJsonSafe(cfg.CLAUDE_JSON) || {};
    const before = claudeJson.mcpServers || {};

    const incoming = {};
    for (const [name, entry] of Object.entries(adaptedAll)) {
      if (crossPlatform && before[name]) {
        report.skipped.push(`mcpServers.${name} (локальное определение для этой ОС сохранено)`);
        continue;
      }
      const e = crossPlatform ? adaptMcpEntry(name, entry, report) : entry;
      if (e) incoming[name] = e;
    }

    if (dryRun) {
      if (Object.keys(incoming).length) {
        report.written.push(`mcpServers → ~/.claude.json: ${Object.keys(incoming).join(', ')}`);
      }
      ...
      const overwritten = Object.keys(incoming).filter(
        (k) => before[k] && JSON.stringify(before[k]) !== JSON.stringify(incoming[k])
      );
      claudeJson.mcpServers = { ...before, ...incoming };
      fs.writeFileSync(cfg.CLAUDE_JSON, JSON.stringify(claudeJson, null, 2) + '\n');
      report.written.push(`mcpServers (${Object.keys(incoming).length} шт.)`);
