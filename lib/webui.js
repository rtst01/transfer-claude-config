...
function runCli(args, extraEnv = {}) {
  return new Promise((resolve) => {
    const p = spawn(cfg.SELF.exec, [...cfg.SELF.args, ...args], { env: { ...process.env, ...extraEnv } });
    let out = '';
...
/** HTML панели: в SEA-бинаре — из встроенного ассета, иначе с диска. */
function getPanelHtml() {
  if (cfg.IS_SEA) {
    return Buffer.from(require('node:sea').getAsset('panel.html')).toString('utf8');
  }
...
function getState() {
  ...
  try {
    ...
    plugins = Object.keys(installed.plugins || {}).sort();
  } catch {}
...
async function handler(req, res) {
  ...
  try {
    ...
    } else if (req.method === 'POST' && (url.pathname === '/api/import' || url.pathname === '/api/diff')) {
      // тело запроса — сам .ccsync файл; diff сравнивает, import применяет
      const buf = await readBody(req);
      const tmp = path.join(os.tmpdir(), `ccsync-upload-${Date.now()}.ccsync`);
      fs.writeFileSync(tmp, buf);
      const args = url.pathname === '/api/diff' ? ['diff', tmp] : ['import', tmp];
      if (url.pathname === '/api/import' && url.searchParams.get('dry') === '1') args.push('--dry-run');
      const pass = req.headers['x-passphrase'];
      const r = await runCli(args, pass ? { CCSYNC_PASSPHRASE: String(pass) } : {});
      fs.unlink(tmp, () => {});
      json(res, 200, r);
