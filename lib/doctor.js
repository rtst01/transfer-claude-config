...
async function doctor() {
  ...
  // плагины: список в репо против локально установленных
  if (state.repoDir && fs.existsSync(path.join(state.repoDir, '.git'))) {
    const repoPlugins = readJsonSafe(path.join(state.repoDir, 'plugins.json'));
    const wanted = repoPlugins && Array.isArray(repoPlugins.plugins) ? repoPlugins.plugins : [];
    if (wanted.length) {
      const installed = readJsonSafe(path.join(cfg.CLAUDE_DIR, 'plugins', 'installed_plugins.json')) || {};
      const local = installed.plugins || {};
      const missing = wanted.filter((key) => !(key in local));
      if (missing.length) {
        const manual = missing.map((k) => 'claude plugin install ' + k).join('; ');
        warn(`плагинов из репо не установлено: ${missing.length}`, `ccsync pull доустановит; вручную: ${manual}`);
      } else {
        ok('плагины синхронизированы с репо', `${wanted.length} шт.`);
      }
    }
  }
