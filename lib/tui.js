...
function header() {
  ...
  console.clear();
  console.log(bold(cyan('  ccsync')) + dim(`  —  Claude Code Sync   ${process.platform} · ${os.hostname().split('.')[0]}`));
  console.log(dim('  ─────────────────────────────────────────────'));
  console.log(`  Репозиторий: ${repoLine}`);
  console.log(
    `  Агенты: ${bold(countDir('agents'))}   Скиллы: ${bold(countDir('skills'))}   ` +
      `Команды: ${bold(countDir('commands'))}   MCP: ${bold(mcp)}`
  );
  console.log(dim('  ─────────────────────────────────────────────'));
}

const MENU = `
  ${bold('1')}. Статус — что изменилось
  ...
  for (;;) {
    ...
    switch (choice) {
      ...
      case '4': {
        ...
        runCli(out ? ['export', cleanPath(out)] : ['export']);
        break;
      }
      case '5': {
